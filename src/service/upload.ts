...
export function buildUploadUrl(req: Request, filename: string): string {
    // Derived from the request, never from an env override: a stale
    // SERVER_ORIGIN (e.g. http://localhost:8000 copied into the production
    // env) would otherwise mint URLs the public client can never load.
    // Local dev yields http://localhost:<port>; Render yields the public
    // host with the https scheme via X-Forwarded-*.
